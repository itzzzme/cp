// Created at: 2025-09-28 08:40:36
...
int solve_dp(int i, vti &v, vti &dp)
{
    if (i >= (int)v.size())
        return INT_MAX;
    if (i == (int)v.size() - 1)
    ...
    if (i + 1 < (int)v.size())
    {
        int curr_cost = abs(v[i] - v[i + 1]);
        one = curr_cost + solve_dp(i + 1, v, dp);
    }

    int two = INT_MAX;
    if (i + 2 < (int)v.size())
    {
        int curr_cost = abs(v[i] - v[i + 2]);
        two = curr_cost + solve_dp(i + 2, v, dp);
    }
